...
(function() {
  var ChosenList, ajaxPaginate, choosePlayer, pietop10, ready, readynoajax, seriesobj,
    __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };

  ready = function() {
    if ($('.top_10_bar').size > 0) {
      return $("div.pagination a").attr('data-remote', 'true');
    }
  };
  ...
  pietop10 = function() {
    ...
    options.series = (function() {
      ...
      _ref = $('#top10').data('players');
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ply = _ref[_i];
        _results.push(seriesobj(ply));
      }
      return _results;
    })();
  ...
  seriesobj = function(ply) {
    return {
      name: ply.name,
      data: [ply.total_points, ply.cost_start, ply.cost_now]
    };
  };
  ...
  choosePlayer = function() {
    var lst;
    lst = new ChosenList;
    $('tbody').on('click', '.person', function(event) {
      event.stopPropagation();
      return lst.add({
        player_id: $(this).children('.player_id').html(),
        player_name: $(this).children('.player_name').html()
      }, this);
    });
    return $('tbody').on('mouseenter', '.splayer', function(event) {
      event.stopPropagation();
      if ($(this).find('.s_id').val()) {
        return $(this).find('input').addClass('cross');
      }
    });
  };
  ...
  $('tbody').on("click", ".pagination a", function() {
  ...
  ChosenList = (function() {
    function ChosenList() {
      this.lastInList = __bind(this.lastInList, this);
      this.chosenlist = [];
    }

    ChosenList.prototype.add = function(ply, clickedperson) {
      var lastchosen;
      if (this.playerInList(ply) && this.lastInList() <= 9) {
        this.chosenlist.push(ply);
        lastchosen = this.lastInList();
        $(clickedperson).effect("transfer", {
          to: $(".splayer" + lastchosen)
        }, 1000);
        $(".s_id" + lastchosen).attr('value', this.chosenlist[lastchosen]["player_id"]);
        return $(".s_name" + lastchosen).attr('value', this.chosenlist[lastchosen]["player_name"]);
      }
    };

    ChosenList.prototype.lastInList = function() {
      return this.chosenlist.length - 1;
    };

    ChosenList.prototype.playerInList = function(ply) {
      var id, ply1, _i, _len, _ref;
      _ref = this.chosenlist;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ply1 = _ref[_i];
        if (ply1.player_id === ply.player_id) {
          id = ply1.player_id;
        }
      }
      return !id;
    ...
    ChosenList.prototype.remove = function() {};

    ChosenList.prototype.initialize = function() {};

    ChosenList.prototype.reorganize = function() {};
...
(function() {
  var frozenhead, orderBy, ready, readynoajax, setupSubmit, translate;
  ...
  frozenhead = function() {
    return $(document).scroll(function() {
      ...
      if (delta > 0) {
        return translate($(".playerTable th"), 0, delta - 2);
      } else {
        return translate($(".playerTable th"), 0, 0);
      }
    });
  };
  ...
  setupSubmit = function() {
    ...
    $('.roundsSlider').slider({
      ...
      slide: function(event, ui) {
        ...
        return $(".rounds_to").val(ui.values[1]);
      }
    });
    $('.priceSlider').slider({
      range: true,
      ...
      slide: function(event, ui) {
        ...
        return $(".price_to").val(ui.values[1] / 10.0);
      }
    });
    $('.submitAlias').on('click', function() {
      $('.player_order').val(orderBy.makeArray());
      $('.scrollingBody').css('color', 'blue').fadeIn();
      return $('.submitFilter').click();
    });
    return $('th').on('click', function(e) {
  ...
  orderBy = {
    ...
    makeArray: function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        for (k in order) {
          v = order[k];
          this.ordersArray.push(v);
        }
      }
      return this.ordersArray;
    },
    combination: function(e, stat) {
      if (e.shiftKey) {
        return this.update(stat);
      } else {
        if (!this.check(stat)) {
          this.remove();
          return this.add(stat);
        }
      }
    },
    remove: function() {
      this.orders = [];
      return $('.statsHeader th i').replaceWith(function() {
    ...
    check: function(stat) {
      var changed, row, _i, _len, _ref;
      changed = false;
      _ref = this.orders;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        row = _ref[_i];
        if (row.name === stat) {
          row.direction = this.changeDirection(row);
          changed = true;
        }
      }
      return changed;
    ...
    arrow: function(order) {
      if (order.direction === 'DESC') {
        return $('.statsHeader th.' + order.name).html("<i class='fa fa-arrow-circle-down fa-fw'></i>" + $('.statsHeader th.' + order.name).text());
      } else {
        return $('.statsHeader th.' + order.name).html("<i class='fa fa-arrow-circle-up fa-fw'></i>" + $('.statsHeader th.' + order.name).text());
      }
    }
  };
...
(function() {
  ...
  ajaxanimation = function() {
    return $('.scrapeform').on('ajax:beforeSend', function(event, xhr, settings) {
      return $('.soccerball').show();
    }).on('ajax:complete', function(event, xhr, status) {
      return $('.soccerball').hide();
    });
  };
